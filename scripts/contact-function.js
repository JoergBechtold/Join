...
function validateEmail(emailInput, container) {
  ...
  if (!isValid && emailInput.value.trim() !== "") {
    if (!err) { err = document.createElement('span'); err.className = 'email-error';
      err.style.color = 'red'; err.textContent = 'Invalid email address';
...
function validatePhone(phoneInput, container) {
  ...
  if (!isValid && phoneInput.value.trim() !== "") {
    if (!err) { err = document.createElement('span'); err.className = 'phone-error';
      err.style.color = 'red'; err.textContent = 'Invalid phone number';
...
function checkInputs() {
  const container = document.querySelector('.container-edit.active') ||
  ...
  const nameIn = container.querySelector('input[placeholder="Name"]'),
        ...
        btn = container.querySelector('.create-btn'),
...
function getInputValues() {
  const container = document.querySelector('.container-edit.active') ||
                    document.querySelector('.container-add.active') || document;
...
function createContactAvatar(name, color) {
  ...
  if (!color) {
    ...
    color = getComputedStyle(document.documentElement)
              .getPropertyValue(randomVar).trim();
  }
  avatar.style.backgroundColor = color;
  avatar.textContent = name.split(" ").map(w => w.charAt(0).toUpperCase()).join("");
  return avatar;
}
...
function showSuccessPopup() {
  ...
  setTimeout(() => { sp.classList.add('popup-hidden'); }, 3000);
}
...
function getRandomColor() {
  ...
  var compColor = getComputedStyle(document.documentElement)
                  .getPropertyValue(rv).trim();
  return compColor;
}
...
function prepareContact(event) {
  ...
  var contactData = { firstname: nameParts.firstName, lastname: nameParts.lastName,
                      email: inputs.email, phone: inputs.phone,
                      contactColor: compColor, initials: nameParts.initials,
...
function finalizeContact(contactData, contactEl) {
  saveContactToFirebase(contactData, function(err, data) {
    if (err) { console.error("Error saving contact:", err); return; }
...
function createContact(event) {
  ...
  if (info) { finalizeContact(info.data, info.el); }
}  
...
function deleteAndCloseEdit() {
  processContactDeletion().then(success => { success ? closeEditContactPopup() : console.error("Deletion failed."); });
}
...
function removeContactFromUI(contactDiv) {
  ...
  if (container.children.length === 0) {
    const line = container.previousElementSibling, lg = line ? line.previousElementSibling : null;
...
function clearDetails() {
  ...
  activeContact = null; dn.textContent = de.textContent = dp.textContent = da.textContent = "";
}
